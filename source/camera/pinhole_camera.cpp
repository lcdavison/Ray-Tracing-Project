...
{
	//	Remap coordinates
	double x = p_pixelx - ( m_viewplane_width * 0.5 );	
	double y = p_pixely - ( m_viewplane_height * 0.5 );

	Vector3 ray_dir ( x - m_position.x, y - m_position.y, m_viewplane_dist );
	ray_dir = ray_dir.x * m_basis [ 0 ] + ray_dir.y * m_basis [ 1 ] + ray_dir.z * m_basis [ 2 ];	//	Rotate ray into world space
	ray_dir.normalize ( );
