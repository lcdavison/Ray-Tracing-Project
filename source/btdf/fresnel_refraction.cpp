...
FresnelRefraction::FresnelRefraction ( ) { }
...
ColourRGB FresnelRefraction::sample_function ( const HitResult& p_hitdata, Vector3& p_transmitted, const Vector3& p_outgoing )
{
	...
	double transmitted_cosine = std::sqrt ( 1.0 - ( refraction_index * refraction_index ) * incident_sine_sqrd );
...
ColourRGB FresnelRefraction::reflectance ( const HitResult& p_hitdata, const Vector3& p_outgoing )
{
	return ColourRGB::BLACK;
}
...
bool FresnelRefraction::total_internal_reflection ( const HitResult& p_hitdata, const Ray& p_ray )
{
	...
	double refractive_index = m_exterior_index / m_interior_index;
